...
variable "deadlock_threshold" {
  type        = number
  description = "Deadlocks threshold as defined by Applications team"
}

variable "failed_connections_threshold" {
  type        = number
  description = "Number of failed connections threshold as defined as Applications team"
}

variable "server_id" {
  type        = string
  description = "Resource ID of the Azure SQL server hosting the database"
}