resource "azurerm_windows_web_app" "app_service_windows" {
  ...
  dynamic "site_config" {
    ...
    content {
      ...
      cors {
        allowed_origins     = lookup(site_config.value, "allowed_origins", null)
        support_credentials = lookup(site_config.value, "support_credentials", null)
      }

      dynamic "virtual_application" {
        for_each = lookup(site_config.value, "virtual_application", null) == null ? [] : [""]
        content {
