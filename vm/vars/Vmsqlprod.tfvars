subscription_id     = "1691759c-bec8-41b8-a5eb-03c57476ffdb"
resource_group_name = "rgsqlprod"
name                = "Vmsqlprod"
os_simple           = "WindowsServer"
size                = "Standard_E8-4s_v3"
image_os            = "windows"
vmimport            = "enabled"
os_disk = {
  caching              = "ReadWrite"
  storage_account_type = "Premium_LRS"
  create               = "FromImage"
  name                 = "Vmsqlprod_OsDisk_1_0b05642e2cb94a3786f78e52ae68712f"
  disk_size_gb         = "127"
  type                 = "Premium_LRS"
}
boot_diagnostics_storage_account_uri = "https://rgsqlproddiag462.blob.core.windows.net/"
boot_diagnostics                     = true
virtual_network_name                 = "vnetdpwglb"
virtual_network_resource_group_name  = "rgvnet"
subnet_name                          = "Production"
new_network_interface = {
  accelerated_networking_enabled = true
  name                           = "vmsqlprod383"
  resource_group_name            = "Rgsqlprod"
  ip_configurations = [
    {
      name                           = "ipconfig1"
      private_ip_address_allocation  = "Static"
      private_ip_address             = "10.253.0.16"
      accelerated_networking_enabled = true
    }
  ]
}
disable_password_authentication = true
identity = {
  type = "SystemAssigned"
}

admin_ssh_keys = [
  {
    public_key = "ssh-rsa "
  }
]

tags = {
  "Application Name" : "Corporate Apps",
  "Application Owner" : "Shaloof/Nowphal",
  "ENV" : "PROD",
  "Creator" : "98e5f3973ad74a6eb7b4ae70c27461ed",
  "Last Backup" : "02/15/2024, 05:07:13 PM"
}


os_profile = {
  computer_name  = "Vmsqlprod"
  admin_username = "localadmin"
}

os_profile_windows_config = {
  enable_automatic_upgrades = true
  provision_vm_agent        = false
}