subscription_id     = "039efcb8-56e2-459b-ad3c-68c086e8feb9"
resource_group_name = "rg-ecommerce-infra-nonprod"
name                = "vm-ecomselfhosted-nonprod"
#os_simple           = ""
size     = "Standard_B4ms"
image_os = "linux"
vmimport = "enabled"
os_disk = {
  caching              = "ReadWrite"
  storage_account_type = "Standard_LRS"
  create               = "FromImage"
  name                 = "vm-ecomselfhosted-nonprod_OsDisk_1_e6381b5ad400428ca7bf72423fc1707a"
  disk_size_gb         = "30"
  type                 = "Standard_LRS"
}
boot_diagnostics_storage_account_uri = ""
boot_diagnostics                     = true
virtual_network_name                 = "vnet-ecommerce-nonprod"
virtual_network_resource_group_name  = "rg-ecommerce-infra-nonprod"
subnet_name                          = "snet-aksnode-dev"
new_network_interface = {
  name                = "vm-ecomselfhosted319"
  resource_group_name = "rg-ecommerce-infra-nonprod"
  ip_configurations = [
    {
      name                          = "ipconfig1"
      private_ip_address_allocation = "Dynamic"
      private_ip_address            = "10.165.80.8"
    }
  ]
}
disable_password_authentication = true
identity = {
  type = "SystemAssigned"
}

admin_ssh_keys = [
  {
    public_key = "ssh-rsa "
  }
]

tags = {
  "Application Name" : "Ecommerce (Dubuy & DM)",
  "Application Owner" : "Shaloof/Kamlesh",
  "ENV" : "DEV"
}
