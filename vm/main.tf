...
resource "azurerm_linux_virtual_machine" "vm_linux" {
  count = var.vmimport == null && local.is_linux ? 1 : 0
  ...
  resource_group_name           = var.resource_group_name
  size                          = var.size
  admin_password                = random_password.current.result
  allow_extension_operations    = var.allow_extension_operations
  availability_set_id           = var.availability_set_id
  capacity_reservation_group_id = var.capacity_reservation_group_id
  # computer_name                   = coalesce(var.computer_name, var.name)
  computer_name                   = substr(replace(coalesce(var.computer_name, var.name), "/\\W|_|\\s/", ""), 0, 15)
...
resource "azurerm_windows_virtual_machine" "vm_windows" {
  count                         = var.vmimport == null && local.is_windows ? 1 : 0
  admin_password                = random_password.current.result
  admin_username                = var.admin_username
  location                      = data.azurerm_resource_group.current.location
  name                          = var.name
...
locals {
  ...
  vm_machine = {
    id                           = try(azurerm_virtual_machine.vm_machine[0].id, null)
    name                         = try(azurerm_virtual_machine.vm_machine[0].name, null)
    network_interface_ids        = try(azurerm_virtual_machine.vm_machine[0].network_interface_ids, null)
    availability_set_id          = try(azurerm_virtual_machine.vm_machine[0].availability_set_id, null)
    proximity_placement_group_id = try(azurerm_virtual_machine.vm_machine[0].proximity_placement_group_id, null)
    timezone                     = null
    identity                     = try(azurerm_virtual_machine.vm_machine[0].identity, null)
  }
}
...
resource "azurerm_network_interface" "vm" {
  ...
  location            = coalesce(var.nic_location, data.azurerm_resource_group.current.location)
  name                = coalesce(var.new_network_interface.name, "${var.name}-nic")
  resource_group_name = coalesce(var.new_network_interface.resource_group_name, var.resource_group_name) ## it needs for import - do not change
  ...
  dynamic "ip_configuration" {
    ...
    content {
      ...
      subnet_id                                          = coalesce(var.subnet_id, data.azurerm_subnet.current.id)
    }
  }
...
resource "azurerm_virtual_machine_data_disk_attachment" "attachment" {
  ...
  virtual_machine_id        = var.vmimport == "enabled" ? local.vm_machine.id : local.virtual_machine.id
  create_option             = each.value.create_option
...
resource "azurerm_virtual_machine" "vm_machine" {
  count                            = var.vmimport == "enabled" && (local.is_linux || local.is_windows) ? 1 : 0
  zones                            = var.zones
  name                             = var.name
  ...
  storage_os_disk {
    name                      = var.os_disk.name
    caching                   = var.os_disk.caching
    create_option             = var.os_disk.create
    managed_disk_type         = var.os_disk.type
    disk_size_gb              = var.os_disk.disk_size_gb
    write_accelerator_enabled = var.os_disk.write_accelerator_enabled
    managed_disk_id           = var.os_disk.managed_disk_id
    image_uri                 = var.os_disk.image_uri
